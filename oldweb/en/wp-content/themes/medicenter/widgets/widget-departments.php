...
class departments_widget extends WP_Widget 
{
    ...
    function widget($args, $instance) 
	{
		...
		if(have_posts()):
			...
			$output .= '<ul class="accordion' . ((int)$active==1 ? ' accordion-active' : '') . '">';
	...
	{	
		...
		<p>
			<label for="<?php echo $this->get_field_id('active'); ?>"><?php _e('First tab opened at start', 'medicenter'); ?></label>
			<select id="<?php echo $this->get_field_id('active'); ?>" name="<?php echo $this->get_field_name('active'); ?>">
				<option <?php echo ((int)$active==1 ? ' selected="selected"':'');?> value='1'><?php _e("yes", 'medicenter'); ?></option>
				<option <?php echo (!(int)$active ? ' selected="selected"':'');?> value='0'><?php _e("no", 'medicenter'); ?></option>
			</select>
		</p>
		<p>
