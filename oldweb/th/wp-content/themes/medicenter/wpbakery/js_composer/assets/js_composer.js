    ...
    $.wpb_composer = {
        ...
        addLastClass:function (dom_tree) {
            ...
            $dom_tree.children(".wpb_sortable").each(function (index) {
                ...
                if (cur_el.hasClass('wpb_vc_column') || cur_el.hasClass('wpb_vc_tabs') || cur_el.hasClass('wpb_vc_nested_tabs') || cur_el.hasClass('wpb_vc_tour') || cur_el.hasClass('wpb_vc_accordion') || cur_el.hasClass('wpb_vc_nested_accordion')) {
...
function wpbTabsInitCallBack(element) {
    element.find('.wpb_tabs_holder:not(.wpb_nested_tabs_holder)').not('.wpb_initialized').each(function (index) {
        ...
        if(!jQuery('.new_element_button', jQuery(this)).length) {
			...
			jQuery(this).find(".tabs_controls:not(.nested_tabs_controls)").append('<li class="add_tab_block"><a href="#new-tab-'+ new_tab_button_id +'" class="add_tab" title="' + i18nLocale.add_tab +'"></a></li>');
        }
...
function wpbNestedTabsInitCallBack(element) {
	 element.find('.wpb_nested_tabs_holder').not('.wpb_initialized').each(function (index) {
		jQuery(this).addClass('wpb_initialized');
        //var tab_counter = 4;
        //
        var $tabs,
        ...
        if(!jQuery('.new_element_button', jQuery(this)).length) {
			jQuery(this).append('<div id="new-tab-' + new_tab_button_id + '" class="new_element_button"></div>');
			jQuery(this).find(".nested_tabs_controls").append('<li class="add_tab_block"><a href="#new-tab-'+ new_tab_button_id +'" class="add_tab" title="' + i18nLocale.add_tab +'"></a></li>');
        }
		$tabs = jQuery(this).tabs({
            panelTemplate: element.find('.wpb_nested_template').html(),
            add:function (event, ui) {
                var tabs_count = jQuery(this).tabs("length") - 2;
                jQuery(this).tabs("select", tabs_count);
                //
            ...
            select: function(event, ui) {
				if(jQuery(ui.tab).hasClass('add_tab')) {
					var $ = jQuery,
                        ...
                        tab_id = (+new Date() + '-' + tabs_count + '-' + Math.floor(Math.random() * 11));
					$tabs.tabs("add", "#tab-" + tab_id, tab_title, $tabs.tabs("length") - 1);
                    $tabs.find('.wpb_vc_nested_tab:last .wpb_vc_param_value[name=tab_id]').val(tab_id);
                    var row = new $.wpbShortcode($tabs.find('.wpb_vc_nested_tab:last'), '1/1', $.wpb_stage);
                    row.init();
                    row.initColumn();
                    row.column._setSortable();
                    wpb_init_tab_controls($tabs, row);
        ...
        jQuery(this).find('.wpb_vc_nested_tab').each(function(){
            var $ = jQuery,
                row = new $.wpbShortcode($(this), '1/1', $.wpb_stage);
            row.init();
            row.initColumn();
...
function wpbAccordionInitCallBack(element) {
    element.find('.wpb_accordion_holder:not(.wpb_nested_accordion_holder)').not('.wpb_initialized').each(function (index) {
		jQuery(this).addClass('wpb_initialized');
        //var tab_counter = 4;
        //
        var $tabs,
...
function wpbNestedAccordionInitCallBack(element) {
    element.find('.wpb_nested_accordion_holder').not('.wpb_initialized').each(function (index) {
		jQuery(this).addClass('wpb_initialized');
        //var tab_counter = 4;
        //
        var $tabs,
        ...
        //
        $tabs = jQuery(that).accordion({
            header:"h3.nested_accordion_header",
            autoHeight:false
        }).sortable({
                axis:"y",
                handle:"h3",
        ...
        jQuery(this).find('.wpb_vc_nested_accordion_tab').each(function(){
            var $ = jQuery,
                row = new $.wpbShortcode($(this), '1/1', $.wpb_stage);
            row.init();
            row.initColumn();
        ...
        $add_btn.unbind('click').click(function(e){
            var $ = jQuery,
                row;
            e.preventDefault();
            $tabs.append($tabs.find('.wpb_nested_template').html());
            $tabs.accordion('destroy').accordion({
                    header:"h3",
                    autoHeight:false
                }).sortable({
            ...
            row = new $.wpbShortcode( $tabs.find('.wpb_vc_nested_accordion_tab:last'), '1/1', $.wpb_stage);
            row.init();
            row.initColumn();
            row.column._setSortable();
            wpb_init_accordion_tab_controls($tabs, row);
