...
*/
    ...
    public function contentAdmin($atts, $content = null) {
        ...
        for ( $i=0; $i < count($width); $i++ ) {
            ...
            $output .= '<h3 class="nested_accordion_header"><a href="#">'.$title.'</a></h3>';
            ...
            $output .= '<div class="wpb_element_wrapper">';
            $output .= '<div class="row-fluid wpb_column_container wpb_sortable_container wpb_'.$this->settings['base'].'_container wpb_container_block wpb_no_content_element_inside">';
...
wpb_map( array(
    "name"		=> __("Nested Accordion tab", "js_composer"),
...
function tab_nested_accordion_title_field($settings, $value) {
...
add_shortcode_param('tab_nested_accordion_title', 'tab_nested_accordion_title_field');

function nested_accordion_tab_id_settings_field($settings, $value) {
    $dependency = vc_generate_dependencies_attributes($settings);
    return '<div class="my_param_block">'
        .'<input name="'.$settings['param_name']
        .'" class="wpb_vc_param_value wpb-textinput '
...
add_shortcode_param('tab_id', 'nested_accordion_tab_id_settings_field');
...
class WPBakeryShortCode_VC_Nested_Accordion extends WPBakeryShortCode {
    ...
    public function contentAdmin( $atts, $content ) {
        ...
        $tmp = '';
        $template = '<div class="wpb_nested_template">'.do_shortcode('[vc_nested_accordion_tab title="New Section" tab_id=""][/vc_nested_accordion_tab]').'</div>';
